...
def make_mount_point(server, share, username):
    ...
    if username:
        ...
        mount_point = os.path.join(
            settings.SMB_MOUNT_ROOT, "motioneye_%s_%s_%s" % (server, share, username)
        )

    else:
        mount_point = os.path.join(
            settings.SMB_MOUNT_ROOT, "motioneye_%s_%s" % (server, share)
        )
...
def list_mounts():
    ...
    with open("/proc/mounts", "r") as f:
        for line in f:
            ...
            logging.debug(
                'found smb mount "//%s/%s" at "%s"' % (server, share, mount_point)
            )
...
def _mount(server, share, smb_ver, username, password):
    ...
    for sec in sec_types:
        ...
        try:
            logging.debug(
                'mounting "//%s/%s" at "%s" (sec=%s)'
                % (server, share, mount_point, sec)
            )
            subprocess.run(
                [
                    "mount.cifs",
        ...
        logging.error(
            'failed to mount smb share "//%s/%s" at "%s"' % (server, share, mount_point)
        )
        return None
...
def _umount(server, share, username):
    ...
    try:
        ...
        logging.error(
            'failed to unmount smb share "//%s/%s" from "%s"'
