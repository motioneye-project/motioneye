...
async def list_cameras(local_config) -> utils.GetCamerasResponse:
    ...
    logging.debug(
        "listing remote cameras on %(url)s"
...
async def get_config(local_config) -> utils.GetConfigResponse:
    ...
    logging.debug(
        "getting config for remote camera %(id)s on %(url)s"
...
async def set_config(local_config, ui_config) -> Union[str, None]:
    ...
    logging.debug(
        "setting config for remote camera %(id)s on %(url)s"
        % {"id": camera_id, "url": pretty_camera_url(local_config)}
    )
...
async def test(local_config, data) -> utils.CommonExternalResponse:
    ...
    what = data["what"]
    logging.debug(
        "testing %(what)s on remote camera %(id)s, on %(url)s"
...
async def list_media(local_config, media_type, prefix) -> utils.ListMediaResponse:
    ...
    logging.debug(
        "getting media list for remote camera %(id)s on %(url)s"
        % {"id": camera_id, "url": pretty_camera_url(local_config)}
    )
...
async def get_zipped_content(
    ...
    logging.debug(
        "downloading zip file for remote camera %(id)s on %(url)s"
        % {"id": camera_id, "url": pretty_camera_url(local_config)}
    )
...
async def make_timelapse_movie(
    ...
    path += (
        "/picture/%(id)s/timelapse/%(group)s/?interval=%(int)s&framerate=%(framerate)s"
...
async def get_timelapse_movie(local_config, key, group) -> utils.CommonExternalResponse:
    ...
    logging.debug(
        "downloading timelapse movie for remote camera %(id)s on %(url)s"
        % {"id": camera_id, "url": pretty_camera_url(local_config)}
    )
...
async def del_media_content(
    ...
    logging.debug(
        "deleting file %(filename)s of remote camera %(id)s on %(url)s"
        % {
            "filename": filename,
            "id": camera_id,
            "url": pretty_camera_url(local_config),
    ...
    if response.error:
        logging.error(
            "failed to delete file %(filename)s of remote camera %(id)s on %(url)s: %(msg)s"
            % {
                "filename": filename,
                "id": camera_id,
                "url": pretty_camera_url(local_config),
