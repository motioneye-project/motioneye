...
class MovieHandler(BaseHandler):
    ...
    async def list(self, camera_id):
        ...
        if utils.is_local_motion_camera(camera_config):
            ...
            if resp.error:
                return self.finish_json(
                    {
                        "error": "Failed to get movie list for %(url)s: %(msg)s."
                        % {
    ...
    async def preview(self, camera_id, filename):
        logging.debug(
            "previewing movie %(filename)s of camera %(id)s"
    ...
    async def delete(self, camera_id, filename):
        logging.debug(
            "deleting movie %(filename)s of camera %(id)s"
            % {"filename": filename, "id": camera_id}
        )
        ...
        if utils.is_local_motion_camera(camera_config):
            ...
            if resp.error:
                return self.finish_json(
                    {
                        "error": "Failed to delete movie from %(url)s: %(msg)s."
                        % {
                            "url": remote.pretty_camera_url(camera_config),
                            "msg": resp.error,
                        }
    ...
    async def delete_all(self, camera_id, group):
        logging.debug(
            'deleting movie group "%(group)s" of camera %(id)s'
        ...
        if utils.is_local_motion_camera(camera_config):
            ...
            if resp.error:
                return self.finish_json(
                    {
                        "error": "Failed to delete movie group at %(url)s: %(msg)s."
                        % {
                            "url": remote.pretty_camera_url(camera_config),
                            "msg": resp.error,
                        }
