...
class PictureHandler(BaseHandler):
    ...
    async def list(self, camera_id):
        ...
        if utils.is_local_motion_camera(camera_config):
            ...
            if resp.error:
                return self.finish_json(
                    {
                        "error": "Failed to get picture list for %(url)s: %(msg)s."
                        % {
    ...
    async def download(self, camera_id, filename):
        logging.debug(
            "downloading picture %(filename)s of camera %(id)s"
        ...
        if utils.is_local_motion_camera(camera_config):
            ...
            if resp.error:
                return self.finish_json(
                    {
                        "error": "Failed to download picture from %(url)s: %(msg)s."
                        % {
                            "url": remote.pretty_camera_url(camera_config),
                            "msg": resp.error,
                        }
    ...
    async def preview(self, camera_id, filename):
        logging.debug(
            "previewing picture %(filename)s of camera %(id)s"
            % {"filename": filename, "id": camera_id}
        )
    ...
    async def delete(self, camera_id, filename):
        logging.debug(
            "deleting picture %(filename)s of camera %(id)s"
            % {"filename": filename, "id": camera_id}
        )
        ...
        if utils.is_local_motion_camera(camera_config):
            ...
            if resp.error:
                return self.finish_json(
                    {
                        "error": "Failed to delete picture from %(url)s: %(msg)s."
                        % {
                            "url": remote.pretty_camera_url(camera_config),
                            "msg": resp.error,
                        }
    ...
    async def zipped(self, camera_id, group):
        ...
        if key:
            logging.debug(
                'serving zip file for group "%(group)s" of camera %(id)s with key %(key)s'
            ...
            if utils.is_local_motion_camera(camera_config):
                ...
                if resp.error:
                    return self.finish_json(
                        {
                            "error": "Failed to download zip file from %(url)s: %(msg)s."
                            % {
                                "url": remote.pretty_camera_url(camera_config),
                                "msg": resp.error,
                            }
            ...
            logging.debug(
                'preparing zip file for group "%(group)s" of camera %(id)s'
            ...
            if utils.is_local_motion_camera(camera_config):
                ...
                logging.debug(
                    'prepared zip file for group "%(group)s" of camera %(id)s with key %(key)s'
                    % {"group": group or "ungrouped", "id": camera_id, "key": key}
                )
                ...
                if resp.error:
                    return self.finish_json(
                        {
                            "error": "Failed to make zip file at %(url)s: %(msg)s."
                            % {
                                "url": remote.pretty_camera_url(camera_config),
                                "msg": resp.error,
                            }
    ...
    async def timelapse(self, camera_id, group):
        ...
        if key:  # download
            logging.debug(
                'serving timelapse movie for group "%(group)s" of camera %(id)s with key %(key)s'
                % {"group": group or "ungrouped", "id": camera_id, "key": key}
            )
            ...
            if utils.is_local_motion_camera(camera_config):
                ...
                pretty_filename = camera_config["camera_name"] + "_" + group
                pretty_filename = re.sub("[^a-zA-Z0-9]", "_", pretty_filename)
                pretty_filename += "." + mediafiles.FFMPEG_EXT_MAPPING.get(
                ...
                if resp.error:
                    msg = (
                        "Failed to download timelapse movie from %(url)s: %(msg)s."
                        % {
                            "url": remote.pretty_camera_url(camera_config),
                            "msg": resp.error,
                        }
            ...
            logging.debug(
                'checking timelapse movie status for group "%(group)s" of camera %(id)s'
                % {"group": group or "ungrouped", "id": camera_id}
            )
            ...
            if utils.is_local_motion_camera(camera_config):
                ...
                if status["progress"] == -1 and status["data"]:
                    ...
                    logging.debug(
                        'prepared timelapse movie for group "%(group)s" of camera %(id)s with key %(key)s'
                        % {"group": group or "ungrouped", "id": camera_id, "key": key}
                    )
                ...
                if resp.error:
                    msg = (
                        "Failed to check timelapse movie progress at %(url)s: %(msg)s."
                        % {
                            "url": remote.pretty_camera_url(camera_config),
                            "msg": resp.error,
                        }
            ...
            if utils.is_local_motion_camera(camera_config):
                ...
                if check_timelapse_resp.error:
                    return self.finish_json(
                        {
                            "error": "Failed to make timelapse movie at %(url)s: %(msg)s."
                            % {
                                "url": remote.pretty_camera_url(camera_config),
    ...
    async def delete_all(self, camera_id, group):
        logging.debug(
            'deleting picture group "%(group)s" of camera %(id)s'
            % {"group": group or "ungrouped", "id": camera_id}
        )
        ...
        if utils.is_local_motion_camera(camera_config):
            ...
            if resp.error:
                return self.finish_json(
                    {
                        "error": "Failed to delete picture group at %(url)s: %(msg)s."
                        % {
                            "url": remote.pretty_camera_url(camera_config),
                            "msg": resp.error,
                        }
