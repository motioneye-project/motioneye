    ...
    async def get_config(self, camera_id):
        if camera_id:
            ...
            if utils.is_local_motion_camera(local_config):
                ...
                if resp.error:
                    msg = (
                        "Failed to get remote camera configuration for %(url)s: %(msg)s."
                        % {
    ...
    async def test(self, camera_id):
        ...
        if utils.is_local_motion_camera(camera_config):
            if what == "upload_service":
                ...
                if result is True:
                    logging.info(
                        "accessing %s succeeded.result %s" % (service_name, result)
                    )
                    return self.finish_json()
                else:
                    logging.warning("accessing %s failed: %s" % (service_name, result))
                ...
                logging.debug(
                    "testing access to network share //%s/%s"
                ...
                try:
                    ...
                    logging.debug(
                        "access to network share //%s/%s succeeded"
                        % (data["server"], data["share"])
                    )
                    return self.finish_json()
