...
async def set_motion_detection(camera_id, enabled):
    ...
    logging.debug(
        "%(what)s motion detection for camera with id %(id)s"
...
async def take_snapshot(camera_id):
    ...
    if resp.error:
        logging.error(
            "failed to take snapshot for camera with id %(id)s: %(msg)s"
            % {"id": camera_id, "msg": utils.pretty_http_error(resp)}
        )

    else:
        logging.debug(
            "successfully took snapshot for camera with id %(id)s" % {"id": camera_id}
        )
