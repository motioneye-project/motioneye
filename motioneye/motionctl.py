    ...
    if settings.MOTION_BINARY:
        ...
        try:
            binary = subprocess.check_output(
                ['which', 'motion'], stderr=utils.DEV_NULL).strip()
...
def take_snapshot(camera_id):
    ...
    logging.debug('taking snapshot for camera with id %(id)s' %
                  {'id': camera_id})
    ...
    def on_response(response):
        if response.error:
            ...
            logging.debug('successfully took snapshot for camera with id %(id)s' % {
                          'id': camera_id})
...
def set_motion_detected(camera_id, motion_detected):
    if motion_detected:
        logging.debug(
            'marking motion detected for camera with id %s' % camera_id)

    else:
        logging.debug(
            'clearing motion detected for camera with id %s' % camera_id)
