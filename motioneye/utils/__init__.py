...
def get_disk_usage(path):
    logging.debug("getting disk usage for path %(path)s..." % {"path": path})
...
def compute_signature(method, path, body: bytes, key):
    ...
    return (
        hashlib.sha1(
            ("%s:%s:%s:%s" % (method, path, body_str or "", key)).encode("utf-8")
        )
        .hexdigest()
        .lower()
    )
...
def build_mask_file_name(camera_id, mask_class):
    file_name = (
        ...
        else "mask_%s_%s.pgm" % (camera_id, mask_class)
    )
...
def parse_editable_mask_file(
    ...
    logging.debug(
        "parsing editable mask %s for camera with id %s: %s"
        % (mask_class, camera_id, file_name)
    )
