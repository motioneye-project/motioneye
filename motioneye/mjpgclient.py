...
class MjpgClient(IOStream):
    ...
    def _check_error(self) -> bool:
        if self.socket is None:
            logging.warning(
                "mjpg client connection for camera %(camera_id)s on port %(port)s is closed"
                % {"port": self._port, "camera_id": self._camera_id}
            )
    ...
    def _error(self, error) -> None:
        logging.error(
            "mjpg client for camera %(camera_id)s on port %(port)s error: %(msg)s"
...
def get_jpg(camera_id):
    if camera_id not in MjpgClient.clients:
        ...
        logging.debug(
            "creating mjpg client for camera %(camera_id)s" % {"camera_id": camera_id}
        )
