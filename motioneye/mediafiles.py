    ...
    for (full_path, st) in _list_media_files(directory, exts):
        ...
        if file_moment < moment:
            ...
            if not listing or len(listing) == len(thumbs):
                ...
                logging.debug(
                    "removing empty directory %(path)s..." % {"path": dir_path}
                )
                try:
...
def cleanup_media(media_type: str) -> None:
    ...
    for camera_id in config.get_camera_ids():
        ...
        preserve_media = camera_config.get(
            "@preserve_%(media_type)ss" % {"media_type": media_type}, 0
        )
        if preserve_media == 0:
        ...
        logging.debug(
            "calling _remove_older_files: %s %s %s"
            % (cloud_enabled, clean_cloud_enabled, clean_cloud_info)
        )
...
def make_movie_preview(camera_config: dict, full_path: str) -> typing.Union[str, None]:
    ...
    logging.debug(
        "creating movie preview for %(path)s with an offset of %(offs)s seconds..."
    ...
    try:
        ...
        logging.error(
            "failed to create movie preview for %(path)s: %(msg)s"
    ...
    try:
        ...
        logging.error(
            "failed to create movie preview for %(path)s" % {"path": full_path}
        )
    ...
    if st.st_size == 0:
        logging.debug(
            "movie probably too short, grabbing first frame from %(path)s..."
            % {"path": full_path}
        )
...
def get_media_content(camera_config, path, media_type):
    ...
    try:
        ...
        logging.error(
            "failed to read file %(path)s: %(msg)s" % {"path": full_path, "msg": str(e)}
        )
        ...
        if _timelapse_process.poll() is None:  # not finished yet
            ...
            if exit_code != 0:
                ...
                try:
                    ...
                    logging.error(
                        'failed to read timelapse movie file "%s": %s'
                        % (tmp_filename, e)
                    )
...
def del_media_content(camera_config, path, media_type):
    ...
    try:
        ...
        logging.error(
            "failed to remove file %(path)s: %(msg)s"
            % {"path": full_path, "msg": str(e)}
        )
