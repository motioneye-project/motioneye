...
class UploadService(object):
    ...
    def upload_file(self, target_dir, filename, camera_name):
        ...
        if target_dir:
            ...
            self.debug(
                'uploading file "%s/%s" to %s' % (target_dir, rel_filename, self)
            )
...
class GoogleDrive(UploadService, GoogleBase):
    ...
    def clean_cloud(self, cloud_dir, local_folders):
        ...
        self.info(
            "found %s/%s folder(s) in local/cloud" % (len(local_folders), len(children))
        )
        ...
        try:
            ...
            if albumsWithName:
                ...
                if count > 0:
                    ...
                    self.debug(
                        'found %s existing album(s) "%s" taking first id "%s"'
                        % (count, name, albumId)
                    )
                    return albumId
            ...
            self.info(
                'Album "%s" was created successfully with id "%s"' % (name, albumId)
            )
            return albumId
...
class SFTP(UploadService):
    ...
    def test_access(self):
        ...
        rm_operations = [
            "RM {}/{}".format(self._location, test_file),
...
def get(camera_id, service_name):
    ...
    if service is None:
        ...
        if cls:
            ...
            logging.debug(
                'created default upload service "%s" for camera with id "%s"'
                % (service_name, camera_id)
            )
...
def upload_media_file(camera_id, camera_name, target_dir, service_name, filename):
    ...
    if not service:
        return logging.error(
            'service "%s" not initialized for camera with id %s'
            % (service_name, camera_id)
        )
    ...
    try:
        ...
        logging.error(
            'failed to upload file "%s" with service %s: %s' % (filename, service, e),
            exc_info=True,
        )
...
def _load():
    ...
    if os.path.exists(file_path):
        ...
        try:
            ...
            logging.error(
                'could not open upload services state file "%s": %s' % (file_path, e)
            )
        ...
        try:
            ...
            logging.error(
                'could not read upload services state from file "%s": %s'
                % (file_path, e)
            )
        ...
        for camera_id, d in list(data.items()):
            for name, state in list(d.items()):
                ...
                if cls:
                    ...
                    logging.debug(
                        'loaded upload service "%s" for camera with id "%s"'
                        % (name, camera_id)
                    )
...
def _save(services):
    ...
    try:
        ...
        logging.error(
            'could not save upload services state to file "%s": %s' % (file_path, e)
        )
...
def clean_cloud(local_dir, data, info):
    ...
    logging.debug(
        "clean_cloud(%s, %s, %s, %s)" % (camera_id, service_name, local_dir, cloud_dir)
    )
